...
- `/src/data/formSchema.json`, `offenseCodes.json`, `mockIncidents.json` — static mock data. `mockIncidents.json` seeds `fir.find_similar_cases` (6 NCRB-style cases). Plan also calls for `mvaFines.json`, `mockRC.json`, `natureCodes.json` (not yet present).
