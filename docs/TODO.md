...
### UI
- [x] `App.tsx` — dashboard shell, registers all 18 tools on mount
...
- [x] All 18 tools register on mount (full surface on load, not per tab); idempotent registration
...
- [x] `src/lib/toolRegistry.ts` — env-agnostic definitions for all 18 tools
...
- [x] Verified `tools/list` (18 tools + schema + readOnly) and `tools/call` (`fill_field` → `validate_form` → `submit`)
...
- **Register all on mount**: all 18 tools on first load (not per tab); idempotent, so per-module calls are safe no-ops
